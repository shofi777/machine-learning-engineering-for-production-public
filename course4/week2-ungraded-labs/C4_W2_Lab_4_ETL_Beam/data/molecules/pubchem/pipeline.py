...
class CountAtoms(beam.DoFn):
  ...
  def process(self, molecule):
    try:
      ...
      # Define result as a dictionary of tensors
      result = {
        'ID': tf.reshape(tf.cast(uid, tf.int64), [-1]),
        'TotalC': tf.reshape(tf.cast(self.count_by_atom_symbol(molecule, 'C'), tf.int64), [-1]),
        'TotalH': tf.reshape(tf.cast(self.count_by_atom_symbol(molecule, 'H'), tf.int64), [-1]),
        'TotalO': tf.reshape(tf.cast(self.count_by_atom_symbol(molecule, 'O'), tf.int64), [-1]),
        'TotalN': tf.reshape(tf.cast(self.count_by_atom_symbol(molecule, 'N'), tf.int64), [-1]),
        'Energy': tf.reshape(tf.cast(label, tf.float32), [-1])
      }
